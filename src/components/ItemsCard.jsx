  ...
  return (
    <div className={`mt-2 py-2 flex w-full flex-none gap-x-4 px-6 ${isTitle ? 'justify-end' : 'even:bg-gray-100'}`}>
      <dt className='flex-none'>
        ...
        <Icon className={`h-6 w-5 ${isTitle ? 'text-green-700' : 'text-gray-400'}`} aria-hidden='true' />
      </dt>
      <dd className={`${ isTitle ? 'text-right text-green-800 font-semibold' : 'text-left w-full'} text-sm leading-6 text-gray-500 truncate`}>{title}</dd>
      <dd className={`${isTitle ? 'text-green-700 font-extrabold' : 'text-gray-500 w-full text-right font-semibold'} text-sm leading-6 text-gray-500`}>$ {value}</dd>
    </div>
  )
}
